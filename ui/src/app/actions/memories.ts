...
export async function createMemory(memoryData: CreateMemoryRequest): Promise<MemoryResponse> {
...
export async function updateMemory(memoryData: UpdateMemoryRequest): Promise<MemoryResponse> {
  return fetchApi<MemoryResponse>(
    `/memories/${memoryData.namespace}/${memoryData.name}`,
    {
      method: 'PUT',
      body: JSON.stringify(memoryData),
    }
